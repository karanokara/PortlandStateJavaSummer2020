...
public class AboutExceptions {
	...
	public void tryCatchFinallyWithVoidRetur(String whatHappened) {
		try {
			whatHappened += "did something dangerous";
			doStuff();
		}
		catch (IOException e) {
			whatHappened += "; the catch block executed";
			return;
		}
		finally {
			whatHappened += ", but so did the finally!";
		}
		
//		assertEquals(whatHappened, "did something dangerous; the catch block executed, but so did the finally");
	}
	
	@Koan
	public void finallyIsAlwaysRa() {
		String whatHappened = new String();
		tryCatchFinallyWithVoidRetur(whatHappened);
		assertEquals(whatHappened, "");			// String not work on pass by reference
	}
